...
def rank(samples: list[dict[str, Any]], websocket_rows: list[dict[str, Any]], exchange_ids: list[str], weights: dict[str,float], market_rows: list[dict[str, Any]] | None = None, window_count: int = 1, required_symbol_count: int = 3) -> list[dict[str, Any]]:
    ...
    dims={
      ...
      "reliability":{e:(70*r["success_rate"]+30*r["ws_success_rate"])/(1+r["ws_disconnects"]/max(r["ws_sessions"],1)) for e,r in raw.items()},
      "market_quality":{e:(0.4*spread_score[e]+0.2*depth_score[e]+0.2*volume_score[e]+0.2*trade_frequency_score[e] if raw[e]["market_symbols"] else 0.0) for e in raw},"market_breadth":_higher_is_better({e:r["market_count"] for e,r in raw.items()}),"accessibility":{e:100*r["success_rate"] for e,r in raw.items()},
    }
    rows=[]
    for ex,r in raw.items():
        ...
        evidence=(r["rest_p95"] is not None, r["ws_first_message_ms"] is not None, r["ws_interval_p95"] is not None, r["ws_symbol_count"] >= required_symbol_count, r["market_symbols"] >= required_symbol_count, r["trade_frequency_hz"] is not None, r["market_count"] is not None)
