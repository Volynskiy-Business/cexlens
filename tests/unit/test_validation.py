...
def test_acceptance_ready_requires_complete_normalized_evidence_and_reports(tmp_path):
    config=AppConfig(
        ...
        probes=ProbeConfig(iterations=1,websocket_observation_seconds=1),
    ...
    with Storage(config.storage_path) as store:
        ...
        for exchange in config.selected_exchanges():
            for probe_type in ("rest_reuse","rest_fresh"):
                store.add_sample(ProbeSample(run_id,exchange,probe_type,"https://example.test",True,utc_now(),duration_ms=10))
            for symbol in config.benchmark_symbols():
                store.add_websocket(WebSocketSummary(run_id,exchange,"wss://example.test",symbol,True,messages=10,observation_seconds=1))
                store.add_orderbook(OrderBookQuality(run_id,exchange,symbol,symbol,True,utc_now()))
    ...
    report_root=tmp_path/"reports"/"campaign-acceptance"; report_root.mkdir(parents=True)
    for name in ("dashboard.html","executive-report.md","rankings.csv","probe_samples.csv","websocket_sessions.csv","market_quality.csv","metric_statistics.csv"):
        (report_root/name).write_text("evidence",encoding="utf-8")
    (report_root/"summary.json").write_text(json.dumps({"recommendation":"binance"}),encoding="utf-8")
