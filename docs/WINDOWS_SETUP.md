...
For restart resilience, run `scripts\install-campaign-watchdog.ps1 -StartDate 2026-08-07`. The watchdog checks every 30 minutes for eight days, uses Task Scheduler's `IgnoreNew` policy, requests wake-to-run, remains eligible on battery power, and the application-level lock exits duplicate daemon launchers safely.
