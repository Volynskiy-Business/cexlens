...
Timestamp quality is never collapsed into an optimistic generic label: reports retain the aggregate category (`VERIFIED`, `MEASURED_OFFSET_OUT_OF_BOUNDS`, `SYNCHRONIZED_OFFSET_UNKNOWN`, `MIXED`, or `UNKNOWN`) and per-category WebSocket session counts.
